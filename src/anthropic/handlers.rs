...
async fn handle_non_stream_request(
    ...
    // 计算缓存模拟字段
    let simulated_cache_read_total = (final_input_tokens as f64 * cache_simulation_ratio) as i32;
    let simulated_cache_creation = (simulated_cache_read_total as f64 * cache_creation_ratio) as i32;
...
///
/// 工作流程（两阶段）：
/// - Phase A：缓冲事件直到收到 contextUsageEvent，期间只发送 ping 保活
/// - Phase B：收到 contextUsageEvent 后 flush 缓冲事件，之后实时流式转发
/// - 流结束时调用 force_flush 兜底（如果 contextUsageEvent 始终没到达）
        ...
        |(mut body_stream, mut ctx, mut decoder, finished, mut ping_interval)| async move {
            ...
            loop {
                tokio::select! {
                    ...
                    // 优先检查 ping 保活
                    _ = ping_interval.tick() => {
                        tracing::trace!("发送 ping 保活事件（延迟启动模式）");
                        let bytes: Vec<Result<Bytes, Infallible>> = vec![Ok(create_ping_sse())];
                        ...
                        match chunk_result {
                            Some(Ok(chunk)) => {
                                ...
                                if !all_events.is_empty() {
                                    // 有事件要发送（Phase A flush 或 Phase B 实时）
                                ...
                                // 空 = 仍在 Phase A 缓冲中，继续读下一个 chunk
                            }
                            Some(Err(e)) => {
                                tracing::error!("读取响应流失败: {}", e);
                                let final_events = ctx.force_flush();
                                let bytes: Vec<Result<Bytes, Infallible>> = final_events
                                    .into_iter()
                                    .map(|e| Ok(Bytes::from(e.to_sse_string())))
                                    .collect();
